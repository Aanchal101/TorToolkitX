...
async def create_quality_menu(
    ...
    for i in unique_formats:
        ...
        text = (
            f"tiny [{human_readable_bytes(unique_formats[i][0])} - {human_readable_bytes(unique_formats[i][1])}] ➡️"
            if i == "tiny"
            else f"{i} [{human_readable_bytes(unique_formats[i][0])} - {human_readable_bytes(unique_formats[i][1])}] ➡️"
        )
